...
def train(**kwargs):
    ...
    for epoch in range(opt.max_epoch):
        print('\n\nepoch = {} / {}'.format(epoch+1, opt.max_epoch))
        start = time.time()
        ...
        if (epoch + 1) % opt.save_every == 0 or epoch == 0: # 每5个epoch保存一次
            prefix = opt.save_prefix+'HRDN_gray_affine_epoch{}_'.format(epoch+1)
        ...
        print('1 epoch time: {:.2f}'.format(time.time() - start),'seconds' ,end = '\t' )
        print('minute = {:.2f}, hours = {:.2f}'.format( ((opt.max_epoch-epoch)*(time.time() - start)/ 60),
                                                        ((opt.max_epoch-epoch)*(time.time() - start)/ 3600)),
              end='\t')


    prefix = opt.save_prefix+'HRDN_gray_affine_'
    file_name = time.strftime(prefix + '%m%d_%H_%M_%S.pth')
    checkpoint = {
        'epoch': epoch + 1,
        "optimizer": optimizer.state_dict(),
