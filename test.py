...
from data.dataset_val_save_image import Val_MoireData,Val_MoireDataLG
...
def test(**kwargs):
    ...
    prefix = "{0}/".format(opt.save_prefix)
    model.eval()
    ...
    for ii, (moires, clears, labels) in tqdm(enumerate(test_dataloader)):
        ...
        for jj in range(bs):
            ...
            label = labels[jj]
            img_path = "{0}{1}_demoire.png".format(prefix, label)
            print('demoire label = ',label)
            img_path2 = "{0}{1}_clean.png".format(prefix, label)
...
def testlg():
    ...
    test_data = Val_MoireDataLG(opt.test_path)
    test_dataloader = DataLoader(test_data,
                                batch_size=opt.test_batch_size,
                                shuffle=False,
                                num_workers=opt.num_workers,
    ...
    for ii, (moires, labels) in tqdm(enumerate(test_dataloader)):
        moires = moires.to(opt.device)
        output_list, _ = model(moires)
        outputs = output_list[0]
        moires = tensor2im(moires)
        ...
        bs = moires.shape[0]
        for jj in range(bs):
            output,moire  = outputs[jj],moires[jj]
            label = labels[jj]
            img_path = "{0}{1}_demoire.png".format(prefix, label)
            img_path2 = "{0}{1}_moire.png".format(prefix, label)
            print('demoire label = ',label)
            print('clean label = ',label)
            save_single_image(output, img_path)
